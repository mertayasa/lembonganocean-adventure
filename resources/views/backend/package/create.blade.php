            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0">Tambah Paket Tour</h5>
                    </div>
                    <div class="card-body">
                        @include('backend.layouts.flash')
                        {!! Form::open(['route' => 'package.store', 'enctype' => 'multipart/form-data', 'id' => 'createPackageForm']) !!}
                        @include('backend.package.form')

                        <div class="mt-3">
                            <a href="{{ route('package.index') }}" class="btn btn-sm btn-danger">Batal</a>
                            <button class="btn btn-sm btn-primary" type="button" x-on:click="$store.package.store($event)">Simpan</button>
                        </div>
                        {!! Form::close() !!}
                    </div>
                </div>
